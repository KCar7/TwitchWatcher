...
class Twitch:
    ...
    async def _watch_loop(self) -> NoReturn:
        ...
        # Cache to store failed watch attempts for a channel
        failed_attempts: dict[int, int] = {}
        # Cache to store active drops by channel for reduced GQL queries
        channel_drops_cache: dict[int, tuple[str, float]] = {}
        
        while True:
            channel: Channel = await self.watching_channel.get()
            current_time = time()
            
            # Reset failed attempts if the channel changes
            if channel.id not in failed_attempts:
                failed_attempts.clear()
                
            # If the channel isn't online anymore, stop watching
            ...
            if not succeeded:
                ...
                failed_attempts[channel.id] = failed_attempts.get(channel.id, 0) + 1
                if failed_attempts[channel.id] >= 3:
                    # After 3 consecutive failures, try to switch to a different channel
                    logger.warning(f"Multiple watch failures for channel: {channel.name}, attempting to switch")
                    self.change_state(State.CHANNEL_SWITCH)
                    failed_attempts[channel.id] = 0
                else:
                ...
                if not self.gui.progress.is_counting():
                    # Only query GQL when necessary - check cache first
                    should_query_gql = True
                    if channel.id in channel_drops_cache:
                        drop_id, last_query_time = channel_drops_cache[channel.id]
                        if current_time - last_query_time < 120:  # 2 minutes
                            # Use cached data if recent enough
                            should_query_gql = False
                            drop = self._drops.get(drop_id)
                            if drop is not None and drop.can_earn(channel):
                                drop.bump_minutes()
                                logger.log(CALL, f"Using cached drop: {drop.name}")
                                should_query_gql = False
                    
                    # If we need to query or cache miss occurred
                    if should_query_gql:
                        handled = False
                        # Solution 1: Query GQL for current drop status
                        try:
                            ...
                            if drop_data is not None:
                                ...
                                channel_drops_cache[channel.id] = (drop_id, current_time)
                                
                                drop = self._drops.get(drop_id)
                                if drop is not None and drop.can_earn(channel):
                                    drop.update_minutes(drop_data["currentMinutesWatched"])
                                    drop_text = f"{drop.name} ({drop.campaign.game}, {drop.current_minutes}/{drop.required_minutes})"
                                    logger.log(CALL, f"Drop progress from GQL: {drop_text}")
                                    handled = True
                        except GQLException:
                            pass  # Fallback to solution 2
                        
                        # Solution 2: Fallback to active drop search if GQL failed
                        if not handled:
                            drop = self.get_active_drop(channel)
                            if drop is not None:
                                drop.bump_minutes()
                                drop_text = f"{drop.name} ({drop.campaign.game}, {drop.current_minutes}/{drop.required_minutes})"
                                logger.log(CALL, f"Drop progress from active search: {drop_text}")
                                
                                # Update cache with this drop
                                channel_drops_cache[channel.id] = (drop.id, current_time)
                            else:
                                logger.log(CALL, "No active drop could be determined")
            
            # Clean up old cache entries to prevent memory leaks
            for ch_id in list(channel_drops_cache.keys()):
                _, last_time = channel_drops_cache[ch_id]
                if current_time - last_time > 600:  # 10 minutes
                    del channel_drops_cache[ch_id]
                    
            # Wait for next watch interval
